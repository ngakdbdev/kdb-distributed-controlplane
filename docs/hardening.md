...
| `KX_BEARER_TOKEN` | `.env` | If you're pulling KDB-X from the KX portal (`KX_INSTALL_SOURCE=kx-portal`) rather than staging a local binary, rotate this in the KX portal if you ever used a value that matches (or was derived from) what's in this repo's history. |
